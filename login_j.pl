...
# my $url = 'https://ga-uk.gto.intranet.db.com:9051/ga2/logon';
	my $url = 'http://wiki.masterdm.ru/pages/viewpage.action?pageId=885293';
...
# 'https://wiki.tools.intranet.db.com/confluence/display/FCL/Production+Support';    #http://www.quakelive.com/#!profile/matches/MartianBuddy';
 ...
 # my $target_dir = shift;
	...
	#	my $target_dir = 'd:/Disks/YandexDisk/work_materials/';
	my $target_dir = 'c:/Users/nmishin/Documents/home/download/';
...
sub download_page {
	...
	if ( $fname =~ /actionpageId=(?<item_number>\w+)/ ) {
		$fname = $+{item_number};
		say "item_number found: $fname";
	}
...
# my $localname='c:/Users/nmishin/Documents/svn/misc/html_extract/t/confl/'.$fname.'.html';
		...
		# $mech->save_url($link,$localname.'2');
		$is_download{$fname}++;
	}
	else {
